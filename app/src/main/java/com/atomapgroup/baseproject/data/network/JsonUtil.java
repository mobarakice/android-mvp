...
public class JsonUtil {
    ...
    /**
     * Invoke generate json object for user login
     *
     * @return
     */
    public static JSONObject getLoginJsonObject(User user) {

        JSONObject object = new JSONObject();

        try {
            JSONObject userObj = new JSONObject();
            userObj.put(Constants.EMAIL, user.getUserId());
            userObj.put(Constants.USER_PASSWORD, user.getPassword());
            object.put(Constants.USER, userObj);
            object.put(Constants.FCM_TOKEN, DataManager.getInstance().getFcmToken());
            object.put(Constants.TERMINAL_ID, DataManager.getInstance().getTerminalId());
